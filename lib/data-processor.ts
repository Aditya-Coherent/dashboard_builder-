...
export function filterData(
  ...
  console.log('🔍 filterData called with aggregationLevel:', filters.aggregationLevel)
  ...
  const filtered = data.filter((record) => {
    ...
    if (filters.aggregationLevel !== undefined && filters.aggregationLevel !== null) {
      // When a specific level is selected, only show records at that level
      const recordLevel = record.aggregation_level
      if (recordLevel !== filters.aggregationLevel) {
        return false
      }
    } else {
    ...
    if (filters.advancedSegments && filters.advancedSegments.length > 0) {
      ...
      if (filters.aggregationLevel === 1) {
        ...
        segmentMatch = filters.advancedSegments.some(seg => {
          ...
          if (filters.aggregationLevel !== null) {
            ...
            // etc.
            if (filters.aggregationLevel === 2 && hierarchy.level_1 === seg.segment) {
              return true
            }
            if (filters.aggregationLevel === 3 && hierarchy.level_2 === seg.segment) {
              return true
            }
            if (filters.aggregationLevel === 4 && hierarchy.level_3 === seg.segment) {
              return true
            }
            if (filters.aggregationLevel === 5 && hierarchy.level_4 === seg.segment) {
              return true
            }
            if (filters.aggregationLevel === 6 && hierarchy.level_5 === seg.segment) {
              return true
            }
          }
  ...
  if (typeof window !== 'undefined') {
    ...
    if (sampleRecord) {
      ...
      const levelMatch = filters.aggregationLevel === null || sampleRecord.aggregation_level === filters.aggregationLevel
      ...
      console.log('🔍 Filter Debug:', {
        ...
        recordsAtSelectedLevel: filters.aggregationLevel !== null 
          ? data
              .filter(r => {
                // First check aggregation level
                if (r.aggregation_level !== filters.aggregationLevel) {
                  return false
                }
    ...
    if (filters.aggregationLevel !== null) {
      const matchingLevelRecords = data.filter(r => 
        r.aggregation_level === filters.aggregationLevel &&
        r.segment_type === filters.segmentType &&
        (filters.geographies.length === 0 || filters.geographies.includes(r.geography))
      ).slice(0, 5)
      
      console.log('🔍 Records at selected level:', {
        level: filters.aggregationLevel,
