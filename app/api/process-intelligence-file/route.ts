 ...
 * Preserves data structure as-is
 */
export async function POST(request: NextRequest) {
  try {
    const formData = await request.formData()
    const intelligenceFile = formData.get('intelligenceFile') as File | null
    const intelligenceType = formData.get('intelligenceType') as string || 'distributor'
    ...
    // Validate file types
    const fileName = intelligenceFile.name.toLowerCase()
    ...
    const fileBuffer = Buffer.from(await intelligenceFile.arrayBuffer())
